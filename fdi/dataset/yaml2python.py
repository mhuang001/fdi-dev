...
from ..utils.loadfiles import get_mmap
...
global glb
MAKE_GEN_PY = False
GENERATED_PY = 'generated.py'
GENERATED = f'\nfrom .{GENERATED_PY[:-3]} import *\n'
gmode = None
...
BEFORE_DATE = f"### Code generated by yaml2python.py. Do not edit below this line."
DATE_LINE = f"# {datetime.now()}"
ADD_ABOVE = f"{indent}# add import lines with correct indentation above this line."
...
{indent}print('ignore', e)
## Code generated by yaml2python.py ends.
"""
Y2P_MOD_MARK2 = '\ntry:\n'
Y2P_MOD_MARK3 = "\nexcept  ImportError as e:\n" + indent + "print('ignore', e)\n### Code generated by yaml2python.py ends.\n"
#

def append_if_not_exists(f, txt, pos='mid'):
    """ __index__.py modifier.
    ...
    txt: str
       text to be written.
    pos: str
       'start', 'mid', and 'end' for the starting text and date, the import lines, and the except and closing line.
    ...
    #text, fileo = get_mmap(f, start=None, access=mmap.ACCESS_READ | mmap.ACCESS_WRITE, close=True, error_msg = '__init__.py' )
    with open(f, 'r') as filew:
        text = filew.read()

    ind1 = text.find(BEFORE_DATE)
    if ind1 >= 0:
        # found the first line. Note that line separatoe can be one or two chars
        indd = ind1 + len(BEFORE_DATE)
        if text[indd+1:].startswith(DATE_LINE) or text[indd+2:].startswith(DATE_LINE):
            # and found the second.
            # do not write back or lose what have just been written
            pass
        else:
            # not find the second line. overwrite from the first line
            text1 = f"{text[0: ind1]}{Y2P_MOD_MARK}"                
            text = text1
    else:
        # not finding the fist line, append the whole
        text1 = f"{text}{Y2P_MOD_MARK}"
        text = text1

    # now we have the completely filled template
    if 0: #pos == 'start':
        with open(f, 'w') as fw:
            fw.write(text)
            return text
    # put the import lines in
    ind2 = text.find(ADD_ABOVE)
    if ind2 >= 0:
        text2 = f"{text[:ind2]}{indent}{txt}\n{text[ind2:]}"
        text = text2
    else:
        msg = '%s file: %s trouble reading' % ('__init__.py with messed up tempplate', f)
        logging.error(msg)
