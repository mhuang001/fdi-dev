...
Y2P_MOD_MARK1 = '### Code generated by yaml2python.py. Do not edit below this line.'
Y2P_MOD_MARK12= '### Code generated by yaml2python.py ends.'
mod = False

def append_if_not_exists(f, txt):
    global mod 
    with open(f, 'a+') as _f:
        for ln in _f.readline():
            if not mod and ln.startswith(Y2P_MOD_MARK1):
                mod = True
            #if ln.startswith(Y2P_MOD_MARK2):
            #    mod = False
            if mod:
                if ln.startswith(txt.strip()):
                    # already has the line
                    return False
        if not mod:
            _f.write(Y2P_MOD_MARK1+'\n')
            mod = True
        _f.write(txt)
        return True
    ...
    for nm in sorted_list:
        ...
        if dry_run:
            ...
            if gmode:
                ...
                # update __init__.py for the first file only.
                gmode = 'w'
                
                if append_if_not_exists(full_path_init, f'# {datetime.now()}\n'):
            ...
            if MAKE_GEN_PY:
                ...
                # write to __init__
                if append_if_not_exists(full_path_init, im):
